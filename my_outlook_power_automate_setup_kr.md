...
Excel Online (Business) 커넥터가 각 행에 자동으로 붙여주는 내부 식별자입니다. 아래 9단계 "행 삭제" 액션에서 **키 열** 드롭다운을 열었을 때 받은시각/보낸사람 같은 실제 열 이름들 말고 **`ItemInternalId`**라는 항목이 따로 보이면 이걸 쓰면 됩니다. 행이 생성되는 순간부터 항상 존재하는 값이라 "예전 행이라 값이 비어있는" 문제 자체가 없습니다. ⚠️ 실제로 이 옵션이 보이는지는 커넥터 버전에 따라 다를 수 있으니 화면에서 먼저 확인하세요.

**B. `메일ID` 열 사용 (ItemInternalId가 안 보일 때만)**

10-1절대로 Excel에 **메일ID** 열을 추가하고 4-3절 "표에 행 추가"에 `Message Id` 매핑을 채우세요. 이 방식은 **그 열을 추가하기 전부터 있던 행은 메일ID가 비어있다**는 문제가 있으므로, 아래 3단계 필터 조건에서 "메일ID가 비어있으면(예전 행) 무조건 삭제 대상"도 같이 처리합니다.
...
1. 트리거: `반복` → 간격 `1`, 빈도 `일`
2. `Excel Online (Business)` → **"테이블에 있는 행 나열"**: 파일/표는 4절과 동일. (앞서 설명한 페이징 설정 꼭 확인)
3. `Data Operation` → **"필터 배열"**: From = 위 액션의 **value** (동적 콘텐츠) → 필터 조건을 입력칸의 **"텍스트 모드로 전환"** 토글로 바꾼 뒤 식으로 직접 입력:
...
4. **(디버깅용, 선택)** `Data Operation` → **"Compose"** 추가 → 입력 = 위 필터 배열 액션의 **body** (동적 콘텐츠) 전체. "테스트" 실행 후 이 Compose의 출력을 열면 **지울 행 전체 목록을 한 번에** 확인할 수 있습니다 (행마다 따로 클릭할 필요 없음).
5. 첨부파일 목록 만들기:
   - `Data Operation` → **"선택"**: From = 3번 필터 배열의 출력, Map을 텍스트 모드로 전환 → 값 = `item()?['첨부파일명']`
   - 첨부파일명 한 칸에 여러 파일이 `, ` (4-2절과 동일 구분자)로 이어져 있을 수 있으므로, 다시 합쳤다 쪼개서 평탄화: `Data Operation` → **"Compose"** 추가 → 입력에 식 `split(join(body('선택'), ', '), ', ')` (실제 화면의 액션 이름이 영문 `Select`로 나온다면 `body('Select')`로)
6. `제어` → **"각각에 적용"**: 위 5번 Compose의 출력 선택 (중첩 없이 이거 하나면 됩니다)
   - `조건`: 빈 문자열이 아닌지 확인 — 식 `not(empty(items('Apply_to_each')))` (첨부파일 없는 행이 섞여 있으면 빈 문자열이 나올 수 있음)
   - 참이면: `OneDrive for Business` → **"경로를 사용하여 파일 메타데이터 가져오기"** (파일 경로 = `concat('/OutlookAttachments/', items('Apply_to_each'))`) → **"파일 삭제"** (파일 = 그 Id). 파일이 이미 없을 수도 있으니 "파일 삭제"의 설정 → 실행 후에 앞 단계의 **"실패함"도 체크**해두세요.
7. 행 삭제 (**A와 B의 방법이 다릅니다** — "행 삭제"는 키 값이 정확히 일치하는 값을 찾는 기능이라, "비어있는 값"을 찾도록 지정할 방법이 없기 때문입니다):

   **A(ItemInternalId)를 쓴다면 (간단, 이 값은 애초에 비어있을 수 없음)**
   - `Data Operation` → **"선택"**: From = 3번 필터 배열의 출력, Map을 텍스트 모드로 전환 → 값 = `item()?['ItemInternalId']`
   - `제어` → **"각각에 적용"** (6번과는 별개의, 역시 중첩 없는 반복): 위 선택의 출력 선택
   - 그 안에 `Excel Online (Business)` → **"행 삭제"**: **키 열**: `ItemInternalId`, **키 값**: `items('Apply_to_each_2')`

   **B(메일ID)를 쓴다면 (메일ID가 비어있는 예전 행은 다른 값으로 대신 지목해야 함)**
   - `제어` → **"각각에 적용"** (6번과는 별개): 이번엔 미리 선택으로 뽑지 말고, **3번 필터 배열의 출력(행 전체)** 을 그대로 선택
   - 그 안에 `제어` → **"조건"**: `not(empty(items('Apply_to_each_2')?['메일ID']))`
     - **예**: `Excel Online (Business)` → **"행 삭제"**: **키 열**: `메일ID`, **키 값**: `items('Apply_to_each_2')?['메일ID']`
     - **아니요**: 메일ID가 없는 행이므로, 그 안에 다시 `제어` → **"조건"**: `not(empty(items('Apply_to_each_2')?['받은시각']))`
       - **예**: `Excel Online (Business)` → **"행 삭제"**: **키 열**: `받은시각`, **키 값**: `items('Apply_to_each_2')?['받은시각']`
       - **아니요**: 메일ID도 받은시각도 둘 다 비어있는 행 — 자동으로 지목할 방법이 없는 아주 오래된 행입니다. 여기는 그냥 비워두고(아무 액션도 없이) 나중에 Excel에서 **수동으로 한 번만** 확인해서 지우세요. 매일 반복되는 흐름을 이런 극소수 예외까지 처리하도록 무리하게 만들 필요는 없습니다.
8. 저장

> ⚠️ "행 삭제"/"파일 삭제"는 되돌릴 수 없습니다. 처음 실행할 때는 3단계 필터 조건을 `addDays(utcNow(), -3650)`처럼 아주 길게 잡아, 4번 Compose로 지울 대상이 의도한 것만 걸리는지 먼저 확인한 뒤 7일로 좁히세요.
