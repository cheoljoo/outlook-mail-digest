...
7. 지울 행의 키 목록 만들기: `Data Operation` → **"선택"**: From = 3번 필터 배열의 출력, Map을 텍스트 모드로 전환 → 값 = `item()?['ItemInternalId']` (B라면 `item()?['메일ID']`)
8. `제어` → **"각각에 적용"** (6번과는 별개의, 역시 중첩 없는 반복): 위 7번 선택의 출력 선택
9. 그 안에 `Excel Online (Business)` → **"행 삭제"**: 파일/표 동일, **키 열**: `ItemInternalId`(B라면 `메일ID`), **키 값**: `items('Apply_to_each_2')`
10. 저장
