...
3. `각각에 적용` (결과 행마다):
   ...
   - 참이면 `Excel Online (Business)` → **"행 삭제"**: 파일/표 동일, **키 열**: `메일ID`, **키 값**: 지금 행의 메일ID
4. 저장

> ⚠️ "행 삭제"는 되돌릴 수 없습니다. 처음 실행할 때는 조건을 `addDays(utcNow(), -3650)`처럼 아주 길게 잡아 "테스트" 화면에서 삭제 대상이 의도한 것만 걸리는지 먼저 확인한 뒤 7일로 좁히세요.
