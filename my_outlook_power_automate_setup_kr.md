...
`ItemInternalId`는 키 열 목록에 없는 것으로 확인되어, **받은시각을 키로 사용**합니다 (초 단위까지 기록되어 실무에서 겹칠 일이 거의 없습니다). 딱 3단계입니다: **① 7일 지난 데이터를 구한다 → ② 그 데이터의 첨부파일을 지운다 → ③ 그 데이터의 행을 지운다.**

1. 트리거: `반복` → 간격 `1`, 빈도 `일`
2. `Excel Online (Business)` → **"테이블에 있는 행 나열"**: 파일/표는 4절과 동일. (앞서 설명한 페이징 설정 꼭 확인)
3. `Data Operation` → **"필터 배열"**: From = 위 액션의 **value** (동적 콘텐츠) → 필터 조건을 **"텍스트 모드로 전환"** 토글로 바꾼 뒤 식으로 직접 입력: `@less(item()?['받은시각'], addDays(utcNow(), -7))` → 결과: 7일 지난 행들만 모인 배열 (① 완료)
4. 첨부파일 지우기 (②):
   ...
   - `제어` → **"각각에 적용"**: 위 Compose의 출력 선택
     - `조건`: 빈 문자열이 아닌지 확인 — `not(empty(items('Apply_to_each')))` (첨부파일 없는 행 때문에 빈 값이 섞일 수 있음)
     - 참이면: `OneDrive for Business` → **"경로를 사용하여 파일 메타데이터 가져오기"** (파일 경로 = `concat('/OutlookAttachments/', items('Apply_to_each'))`) → **"파일 삭제"** (파일 = 그 Id, 실행 후에 앞 단계 "실패함"도 체크 — 파일이 이미 없을 수도 있음)
5. 행 지우기 (③):
   - `Data Operation` → **"선택"**: From = 3번 필터 배열의 출력, Map을 텍스트 모드로 전환 → 값 = `item()?['받은시각']`
   - `제어` → **"각각에 적용"** (4번과는 별개의, 중첩 없는 반복): 위 선택의 출력 선택
     - `Excel Online (Business)` → **"행 삭제"**: **키 열**: `받은시각`, **키 값**: `items('Apply_to_each_2')`
6. 저장

> ⚠️ "행 삭제"/"파일 삭제"는 되돌릴 수 없습니다. 처음 실행할 때는 3단계 필터 조건을 `addDays(utcNow(), -3650)`처럼 아주 길게 잡아 "테스트"로 삭제 대상이 의도한 것만 걸리는지 먼저 확인한 뒤 7일로 좁히세요. 받은시각이 비어있는 극소수 행(실측 484행 중 2개)은 이 방식으로 못 지우니 발견되면 수동으로 지우면 됩니다.
