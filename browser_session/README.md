...
## 알려진 제약: 파일 다운로드는 조직 정책으로 차단됨

실제로 로그인 후 SharePoint의 `OutlookDigest.xlsx`를 열어보면, 화면은 정상적으로 렌더링되어
스크린샷/HTML로 내용을 읽는 것은 됩니다. 하지만 File 메뉴에 다운로드/인쇄/다른 이름으로 저장
항목 자체가 없고, 화면에 **"조직에서 이 장치를 사용한 다운로드, 인쇄 또는 동기화를 허용하지
않습니다"** 라는 안내가 뜹니다 — Conditional Access/DLP 정책이 이 Linux 서버를 "관리되지 않는
기기(비준수 device)"로 인식해서 다운로드 자체를 막는 것입니다. 이 프로젝트에서 지금까지 만난
다른 차단(관리자 동의 필요, 익명 링크 없음 등)과 같은 종류의 제약입니다.

`download_file.py`도 이 경우를 감지해서 안내 메시지를 출력하도록 되어 있습니다.

**결론**: 이 방식으로는 진짜 `.xlsx` 바이너리 파일을 직접 받을 수는 없고, `fetch_page.py`로
**화면을 읽는 용도**로만 쓸 수 있습니다. 실제 파일 자체가 필요하면 여전히
[my_outlook_power_automate_setup.md](../my_outlook_power_automate_setup.md) 7절의
OneDrive 동기화 + Windows 작업 스케줄러 + scp 경로가 필요합니다.
